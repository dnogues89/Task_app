...
class ActualizarPreventa(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form: BaseModelForm) -> HttpResponse:
        ...
        #             asignacion_tareas.crear_tarea(pv.user,pv,'cedula azul')
        ...
        #             asignacion_tareas.crear_tarea(pv.user,pv,'socio persona fisica')
                    
        #     if pv.co_titular == 'Si':
        #         antes = Tareas.objects.filter(pv=pv,titulo='CO-Titular DNI FRENTE').count()
        #         if antes == 0:
        #             asignacion_tareas.crear_tarea(pv.user,pv,'cotitulares')
