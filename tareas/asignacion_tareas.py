...
def crear_tareas_usuario(usuario,preventa):
    tipo_tarea = TipoTarea.objects.get(tipo='tareas por usuario')
    tareas = AsignacionTareas.objects.filter(tipo=tipo_tarea)
    for tarea in tareas:
        objeto = Tareas.objects.create(user = usuario, titulo = tarea.titulo, descripcion=tarea.descripcion, descarga=tarea.descarga, tipo_doc = tarea.tipo_doc, pv=preventa,tipo_tarea=tipo_tarea).save()
